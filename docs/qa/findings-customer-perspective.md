...
- **Still open — needs pipeline/data work, not a frontend change:** H2 (Shippo's bogus leadership rows — people-extraction validation + re-enrich), H3 root (strip the bad competitor row at source), M1 (industry-taxonomy normalization), M4 (funding-coverage backfill), G2 (thin profiles), G4 / G5 (investor website + description data), L4 (Bron help-center URL), M9 (label individual investors). Minor cosmetic Lows (L1/L2/L5/L6/L7/L10/L11/L12) deferred.
