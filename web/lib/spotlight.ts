...
const POOL_SIZE = 10;
// Eligibility (description_short present) is applied by the display fetch, so
// rank more candidates than the pool needs to absorb the ineligible ones.
 ...
 * top eligible companies (description present), fill from newest companies if
 * short, then shuffle with today's UTC-date seed. Returns [] when Supabase is
 * unconfigured or the index is empty — the page renders a fallback.
 */
export async function buildSpotlightPool(
  ...
  const [roundsResult, newsResult, freshResult] = await Promise.all([
    supabase
      .from("funding_rounds")
  ...
  if (candidateIds.length > 0) {
    ...
    const rows = (data ?? []) as unknown as SpotlightCompanyRow[];
    rows.sort((a, b) => {
